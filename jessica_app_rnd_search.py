...
st.set_page_config(layout="wide", page_title="Jessicka Rotation – Complete Analysis")
...
Displays **all key visuals**: SSRN baseline, variance reduction, decay curve, theta sensitivity, ablation, and infographic.
""")

# ------------------------------------------------------------
# 1. Helper functions (no external files needed)
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# 2. Fast evaluation (used during random search, on subset)
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# 3. Full evaluation (on all paths, for final metrics and visuals)
# ------------------------------------------------------------
def full_evaluation(all_returns, all_vols, bh_sharpes, params):
    ...
    # Ablation study (on subset for speed)
...
if run_button:
    ...
    # Step 3: final evaluation on all paths (generates all visuals)
    ...
    # Panel A: Variance reduction bar
    ...
    axes[1,0].plot(steps, mean_sig, 'b-', label='Empirical mean')
    axes[1,0].fill_between(steps, p10, p90, alpha=0.2, color='blue', label='10th-90th percentile')
    ...
    axes[1,0].set_title('Power‑law decay')
    axes[1,0].legend()
    ...
    axes[1,1].set_title('Sensitivity to θ')
    axes[1,1].legend()
    plt.tight_layout()
    st.pyplot(fig)
    ...
    ax_abl.set_ylabel('Mean Sharpe')
    ax_abl.set_title('Ablation study')
    st.pyplot(fig_abl)

    # ---------- Stunning infographic (violin + variance bar + radar + table) ----------
    ...
    axs[0,0].set_xticklabels(['Baseline', 'Jessicka'])
    ...
    bh_sharpes_arr = np.array(bh_sharpes)
    rot_sharpes_arr = np.array(rot_sharpes)
    bh_vals = [bh_mean, 1 / bh_var, -np.mean(bh_dds), np.mean(bh_sharpes_arr > 0)]
    rot_vals = [rot_mean, 1 / rot_var, -np.mean(rot_dds), np.mean(rot_sharpes_arr > 0)]
    ...
    axs[1,0].set_title('Radar (higher is better)')
    ...
    table_data = [
        ...
        ['Win Rate', f'{np.mean(bh_sharpes_arr > 0):.0%}', f'{np.mean(rot_sharpes_arr > 0):.0%}', '↑']
    ]
